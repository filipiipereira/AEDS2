...
bool palindromo(char* string) //Função que verifica se uma string é um palíndromo
{
    ...
    {
        if(string[i] != string[tamanhoString(string) - 1 - i] || !(string[i] >= 'a' &&  string[i] <= 'z') && !(string[tamanhoString(string) - 1 - i] >= 'A' && string[tamanhoString(string) - 1 - i] <= 'Z')) //Compara o caractere da posição i com o caractere da posição correspondente do final
        {
