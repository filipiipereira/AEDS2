...
int getMenorFilho(Pokemon *array, int i, int tamHeap) {
    int filho;
    if (2 * i == tamHeap || 
        array[2 * i].height < array[2 * i + 1].height || 
        (array[2 * i].height == array[2 * i + 1].height && strcmp(array[2 * i].name, array[2 * i + 1].name) < 0)) {
        filho = 2 * i;
    } else {
        filho = 2 * i + 1;
    }
...
void reconstruir(Pokemon *array, int tamHeap) {
    int i = 1;
    while (i <= (tamHeap / 2)) {
        int filho = getMenorFilho(array, i, tamHeap);
...
int main() {
    ...
    heapSortParcial(pokemonsSelecionados, quantidadePokemonsSelecionados);
    for(int i = 0; i < 10; i++){
